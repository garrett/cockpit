...
export function make_partition_table_page(parent, block, next_card) {
    ...
    const ptable_card = new_card({
        title: _("Partition table"),
        next: next_card,
        page_block: block,
        component: PartitionTableCard,
...
const PartitionTableCard = ({ card, block, block_ptable }) => {
    return (
        <StorageCard card={card}>
            <CardBody>
                <DescriptionList className="pf-m-horizontal-on-sm">
                    <StorageDescription title={_("Table type")} value={block_ptable.Type} />
                </DescriptionList>
            </CardBody>
        </StorageCard>);
};
